...
$exceldata = array();
/*/DELETE TABLE ALREADY EXISTS
$table = "theory_output";

if(mysqli_num_rows(mysqli_query($db,"SHOW TABLES LIKE '".$table."'"))==1) 
{
    $qu = "DROP TABLE `theory_output`";
    if (mysqli_query($db, $qu)) {
    echo "Done...!";
  } else {
    echo "Error: " . $qu . "<br>" . mysqli_error($db);
  }

}

$sqlc = "CREATE TABLE `theory_output` (
...
if (mysqli_query($db, $sqlc)) {
    ...
    echo "Error: " . $sqlc . "<br>" . mysqli_error($db);
  }*/
// Create connection
...
{ 
  ...
  $sql = "INSERT INTO teacher_theory (T_Name, T_Course_Code, T_Year, T_Semester, T_division)
      VALUES ('".$rowData[0][0]."', '".$rowData[0][1]."', '".$rowData[0][2]."',  '".$rowData[0][3]."', '".$rowData[0][4]."')";
    $cnt = $cnt + 1; 
  if (mysqli_query($db, $sql)) {
    //$exceldata[] = $rowData[0];
    for($cnt = 1; $cnt<=9;$cnt++){
    $sql_out = "INSERT INTO theory_output (T_Name , T_Course_Code, T_Year , T_Semester,  T_Division , Q_id , tot_cont)
      VALUES ('".$rowData[0][0]."', '".$rowData[0][1]."', '".$rowData[0][2]."',  '".$rowData[0][3]."', '".$rowData[0][4]."', '$cnt', '0')";
      if (mysqli_query($db, $sql_out)) {
      }
      else {
    echo "Error: " . $sql_out . "<br>" . mysqli_error($db);
  }
    }
...
*/
mysqli_close($db);
header('Location: add_teachers_prac.php');
    exit();
